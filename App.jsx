...
const BG = "#FFFFFF", BGS = "#F7F6F3";
const TX = "#1A1A1A", TM = "#555555", TL = "#999999", BO = "#E5E2DC";
const FF = "'DM Sans', sans-serif";

// zoom lock
const _vp = document.querySelector('meta[name="viewport"]');
if (_vp) _vp.content = "width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no";
...
_gs.textContent = `
html,body{scroll-behavior:smooth;touch-action:pan-x pan-y}
*{-webkit-tap-highlight-color:transparent;box-sizing:border-box}
@keyframes pulseLock{0%,100%{transform:scale(1)}50%{transform:scale(1.15)}}
@keyframes fadeUp{from{opacity:0;transform:translateY(30px)}to{opacity:1;transform:translateY(0)}}
@keyframes fadeR{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:translateY(0)}}
@keyframes confettiFall{0%{opacity:1;transform:translateY(0) rotate(0deg) scale(1)}100%{opacity:0;transform:translateY(220px) rotate(720deg) scale(0.4)}}
...
@keyframes auraBurst{0%{opacity:.7;transform:scale(.85)}40%{opacity:.35;transform:scale(1.06)}100%{opacity:0;transform:scale(1.22)}}
@keyframes scanline{0%{top:0%;opacity:.6}80%{opacity:.6}100%{top:100%;opacity:0}}
@keyframes rippleOut{0%{opacity:.6;transform:scale(1)}100%{opacity:0;transform:scale(2.8)}}
@keyframes gc1{0%,100%{clip-path:inset(0 0 95% 0);transform:translate(-2px,0)}50%{clip-path:inset(30% 0 50% 0);transform:translate(2px,0)}}
@keyframes gc2{0%,100%{clip-path:inset(60% 0 20% 0);transform:translate(2px,0)}50%{clip-path:inset(80% 0 5% 0);transform:translate(-2px,0)}}
@keyframes d0{0%,100%{transform:translate(0,0)}50%{transform:translate(14px,-22px)}}
@keyframes d1{0%,100%{transform:translate(0,0)}50%{transform:translate(-18px,16px)}}
@keyframes d2{0%,100%{transform:translate(0,0)}50%{transform:translate(22px,10px)}}
@keyframes d3{0%,100%{transform:translate(0,0)}50%{transform:translate(-10px,-18px)}}
@keyframes d4{0%,100%{transform:translate(0,0)}50%{transform:translate(16px,20px)}}
@keyframes d5{0%,100%{transform:translate(0,0)}50%{transform:translate(-20px,8px)}}
@keyframes cardPulse{0%{box-shadow:0 0 0 0 rgba(74,124,89,.45)}50%{box-shadow:0 0 0 10px rgba(74,124,89,.1)}100%{box-shadow:0 0 0 0 rgba(74,124,89,0)}}
@keyframes bannerIn{from{opacity:0;transform:translateY(18px) scale(.96)}to{opacity:1;transform:translateY(0) scale(1)}}
.bp:active{transform:scale(.95)!important;transition:transform .1s!important}
`;
...
function useVis(ref){
  ...
  useEffect(()=>{
    ...
    const obs=new IntersectionObserver(([e])=>{
      ...
      if(r>.2&&!entered){setEntered(true);fire();}
      if(r>.5&&prev.current<=.5)fire();
...
function useActiveStep(){
  ...
  useEffect(()=>{
    ...
    const h=()=>{let f=0;ids.forEach((id,i)=>{const el=document.getElementById(id);if(el&&el.getBoundingClientRect().top<window.innerHeight*.65)f=i+1;});setA(f);};
    window.addEventListener("scroll",h,{passive:true});return()=>window.removeEventListener("scroll",h);
  },[]);return a;
}

function useCountdown(){
  const s=useRef(Date.now()),[r,setR]=useState(48*3600);
  useEffect(()=>{const iv=setInterval(()=>setR(Math.max(0,48*3600-Math.floor((Date.now()-s.current)/1e3))),1000);return()=>clearInterval(iv);},[]);
...
function Counter({target,dur=2000}){
  ...
  useEffect(()=>{const el=ref.current;if(!el)return;const o=new IntersectionObserver(([e])=>{if(e.isIntersecting){setVis(true);o.disconnect();}});o.observe(el);return()=>o.disconnect();},[]);
  useEffect(()=>{if(!vis||st.current)return;st.current=true;const s=performance.now();const t=n=>{const p=Math.min(1,(n-s)/dur);setV(Math.round((1-Math.pow(1-p,3))*target));if(p<1)requestAnimationFrame(t);};requestAnimationFrame(t);},[vis,target,dur]);
...
function CopyBtn({text,label,onCopied}){
  ...
  const go=async()=>{
    ...
    setTimeout(()=>setCopied(false),2500);setTimeout(()=>setGlow(false),1000);
...
function CopyRow({label,sublabel,value,copyValue,large,onCopied,isSlot,amount}){
  ...
  const handle=()=>{setBadge(true);setPulse(true);setTimeout(()=>setBadge(false),2800);setTimeout(()=>setPulse(false),600);if(onCopied)onCopied();};
  return(
    <div>
...
function PaymentPage({data}){
  const [show,setShow]=useState(false),[confetti,setConfetti]=useState(false);
  const pct=useScrollPct(),activeStep=useActiveStep(),countdown=useCountdown();
  const [cIban,setCIban]=useState(false),[cAmt,setCamt]=useState(false),[cRef,setCref]=useState(false);
  ...
  const handleWa=e=>{e.preventDefault();setConfetti(true);setTimeout(()=>{setConfetti(false);window.open(waLink,"_blank");},600);};
        ...
        <div style={{width:"100%",maxWidth:"460px"}}>
          ...
          <div style={{background:BG,border:`1px solid ${BO}`,borderRadius:"16px",padding:"20px 18px",marginBottom:"16px"}}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:"6px",marginBottom:"14px"}}>
              {[{icon:"🔒",label:"Paiement sécurisé",anim:"pulseLock 2.5s ease infinite"},{icon:"🏢",label:"Entreprise européenne"},{icon:"✓",label:<>+<Counter target={4000}/> clients</>}].map((b,i)=>(
                <span key={i} style={{display:"flex",alignItems:"center",gap:"5px",padding:"5px 10px",borderRadius:"20px",background:GL,fontSize:"11px",color:GD,fontWeight:600}}>
                  <span style={{fontSize:"12px",animation:b.anim||"none"}}>{b.icon}</span>{b.label}
                </span>
              ))}
            </div>
          ...
          <div style={{position:"relative"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:"16px"}}>
              ...
              <StepCard n={1} icon="🏦" iconBg="#E3F1E8" title="Ouvrez votre application bancaire" id="s1"
                desc="Ouvrez l'app de votre banque et cherchez le bouton pour faire un virement SEPA.">
              ...
              <StepCard n={2} icon="🔢" iconBg="#E6F0FA" title="Copiez et collez l'IBAN" id="s2"
                desc="C'est le numéro du compte vers lequel vous envoyez l'argent.">
                <CopyRow label="IBAN" value="LU80 4080 0000 4547 7817" copyValue="LU804080000045477817" onCopied={()=>{setCIban(true);scrollTo("s3");}}/>
                <Tip icon="🇱🇺" text="Si votre banque demande un pays, sélectionnez Luxembourg — c'est là où le compte est domicilié."/>
              </StepCard>

              <StepCard n={3} icon="👤" iconBg="#F3EDE4" title="Entrez le nom du destinataire" id="s3"
                desc="Ce champ peut s'appeler :">
              ...
              <StepCard n={4} icon="💶" iconBg="#E8F0EB" title="Entrez le montant exact" id="s4"
              ...
              <StepCard n={5} icon="✏️" iconBg="#F0EDE6" title="En référence, mettez votre nom" id="s5"
                desc="Ce champ peut s'appeler :">
              ...
              <StepCard n={6} icon="✅" iconBg={GL} title="Confirmez et envoyez-nous la capture" id="s6" highlight
                desc="Appuyez sur Confirmer dans votre app, puis envoyez-nous la capture sur WhatsApp.">
                ...
                <a href={waLink} onClick={handleWa} className="bp" style={{display:"flex",alignItems:"center",justifyContent:"center",gap:"8px",background:"#25D366",color:"#fff",borderRadius:"12px",padding:"14px",fontSize:"14px",fontWeight:700,textDecoration:"none",fontFamily:FF}}>
                  {WA_SVG} Envoyer la capture sur WhatsApp
                </a>
              </StepCard>
            </div>
          </div>
        ...
        {[{label:"Prénom et nom du client",value:name,set:setName,ph:"Jean Dupont"},{label:"Montant (€)",value:amount,set:setAmount,ph:"178"},{label:"N° commande (optionnel)",value:order,set:setOrder,ph:"QS-4521"}].map(f=>(
          <div key={f.label} style={{marginBottom:"16px"}}>
            ...
            <input value={f.value} onChange={e=>f.set(e.target.value)} placeholder={f.ph}
              ...
              onFocus={e=>e.target.style.borderColor=GREEN} onBlur={e=>e.target.style.borderColor=BO}/>
          </div>
        ))}
